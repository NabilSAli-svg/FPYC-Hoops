    ...
    <section style={{ background: 'var(--bone)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
        <div style={{
          ...
          <WIcon name="megaphone" size={12}/> Heads up
        </div>
...
window.Schedule = function Schedule() {
  const games = [
    ...
    <section id="schedule" style={{ maxWidth: 1200, margin: '0 auto', padding: '64px 24px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={btn('ghost')}><WIcon name="filter" size={14}/> All teams</button>
          <button style={btn('gold')}>Full schedule <WIcon name="arrow-right" size={14}/></button>
        </div>
      </div>
          ...
          <div key={i} style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--fg-soft)', marginTop: 4, display: 'inline-flex', gap: 4, alignItems: 'center' }}>
                <WIcon name="map-pin" size={12}/>{g.loc}
              </div>
            </div>
            <span style={{
            ...
            <a href="#" style={{ color: 'var(--court-navy)', fontSize: 13, fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 }}>Details <WIcon name="arrow-right" size={14}/></a>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section id="news" style={{ maxWidth: 1200, margin: '0 auto', padding: '88px 24px 0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginTop: 32 }}>
        {posts.map((p, i) => (
          <article key={i} style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 2px rgba(10,31,61,0.06)' }}>
            <div style={{
              height: 180,
              ...
              display: 'flex', alignItems: 'flex-end', padding: 18, color: p.img === 'gold' ? 'var(--court-navy)' : '#fff',
            }}>
              <div style={{ position: 'absolute', right: -20, top: -20, opacity: 0.18 }}>
                <img src="../../assets/basketball-glyph.svg" alt="" style={{ width: 180 }}/>
              </div>
              <span style={{ fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: 999, background: 'rgba(0,0,0,0.18)' }}>{p.eyebrow}</span>
            </div>
...
window.FaqContact = function FaqContact() {
  return (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 56 }}>
        ...
        <aside id="volunteer" style={{ background: 'var(--court-navy)', color: '#fff', borderRadius: 14, padding: 32, alignSelf: 'start', position: 'sticky', top: 92 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 22 }}>
            <Contact icon="phone" label="Commissioner of Basketball" value="(703) 425-7000" />
            <Contact icon="mail"  label="Email" value="basketball@fpycsports.com" />
            <Contact icon="map-pin" label="Office" value="3955 Pickett Rd, Fairfax VA" />
          </div>
          <a href="#" style={{
          ...
          }}>I want to coach <WIcon name="arrow-right" size={14}/></a>
        </aside>
      </div>
    </section>
  );
  ...
  return (
    <div style={{ borderTop: '1px solid var(--border)', padding: '18px 0' }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <span>{q}</span>
        <WIcon name={open ? 'minus' : 'plus'} size={18} color="var(--court-navy)"/>
      </button>
      {open && <p style={{ marginTop: 12, fontSize: 14, color: 'var(--fg-soft)', lineHeight: 1.6, textWrap: 'pretty' }}>{a}</p>}
    </div>
  );
}
...
function Contact({ icon, label, value }) {
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
      <WIcon name={icon} size={16} color="var(--varsity-gold)" style={{ marginTop: 3 }}/>
      <div>
...
function btn(kind) {
  const base = { fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 13, padding: '10px 14px', borderRadius: 8, cursor: 'pointer', border: '1px solid transparent', display: 'inline-flex', alignItems: 'center', gap: 6 };
  if (kind === 'gold') return { ...base, background: 'var(--varsity-gold)', color: 'var(--court-navy)' };
  return { ...base, background: 'transparent', color: 'var(--court-navy)', borderColor: 'var(--court-navy)' };
}
