...
window.Programs = function Programs() {
  return (
...
function ProgramCard({ tag, title, grades, price, desc, bullets, featured }) {
  return (
    <div style={{
      ...
      borderRadius: 12,
      overflow: 'hidden',
      boxShadow: featured ? '0 16px 40px rgba(10,31,61,0.18)' : '0 1px 2px rgba(10,31,61,0.06)',
      ...
      <div style={{ padding: '22px 24px 24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <p style={{ fontSize: 14, lineHeight: 1.55, color: featured ? 'rgba(255,255,255,0.82)' : 'var(--fg-soft)', marginTop: 14, marginBottom: 18, textWrap: 'pretty' }}>{desc}</p>
        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 24px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {bullets.map(b => (
            <li key={b} style={{ display: 'flex', gap: 8, fontSize: 13, color: featured ? 'rgba(255,255,255,0.85)' : 'var(--fg)' }}>
              <WIcon name="check" size={14} color="var(--varsity-gold)" style={{ marginTop: 3, flexShrink: 0 }}/>
              <span>{b}</span>
            </li>
          ))}
        </ul>
        <a href="#register" style={{
          ...
          color: featured ? 'var(--court-navy)' : 'var(--court-navy)',
        ...
        }}>Sign up <WIcon name="arrow-right" size={14}/></a>
      </div>
    </div>
  );
}

window.SectionHead = function SectionHead({ eyebrow, title, sub }) {
  return (
    <div>
      ...
      {sub ? <p style={{ fontSize: 16, color: 'var(--fg-soft)', marginTop: 12, maxWidth: 640, lineHeight: 1.55 }}>{sub}</p> : null}
    </div>
  );
};
