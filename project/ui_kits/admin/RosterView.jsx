...
window.RosterView = function RosterView({ team, players, onAdd }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Card padding={0}>
        <div style={{
          ...
          <div>#</div><div>Player</div><div>Grade</div><div>School</div><div>Guardian</div><div>Waiver</div><div>Status</div><div></div>
        </div>
        {filtered.map((p, i) => (
          <div key={p.id} style={{
            display: 'grid',
            ...
            <div>{p.waiver ? <span style={{ color: 'var(--status-win)', display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 13, fontWeight: 700 }}><Icon name="check-circle-2" size={14}/>On file</span> : <span style={{ color: 'var(--foul-red)', display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 13, fontWeight: 700 }}><Icon name="alert-circle" size={14}/>Missing</span>}</div>
            <div><Pill kind={p.status === 'active' ? 'navy' : p.status === 'pending' ? 'warn' : 'neutral'}>{p.status}</Pill></div>
            <div style={{ textAlign: 'right' }}><Icon name="more-horizontal" size={18} color="var(--fg-muted)" /></div>
          </div>
        ))}
      </Card>
    </div>
