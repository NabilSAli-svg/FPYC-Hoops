...
window.DashboardView = function DashboardView({ team, players, games, onGo }) {
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 20 }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, fontSize: 13, color: 'rgba(255,255,255,0.78)' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><Icon name="map-pin" size={14}/>{next.location}</span>
              <span style={{ width: 3, height: 3, borderRadius: '50%', background: 'rgba(255,255,255,0.4)' }} />
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><Icon name="clock" size={14}/>{next.time}</span>
            </div>
          </div>
          <div style={{
      ...
      <Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 4 }}>
          {recent.map(g => (
            <ResultRow key={g.id} game={g} teamName={team.name} />
          ))}
        </div>
      </Card>
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 12, marginBottom: 4 }}>
      <div>
        ...
        {subtitle ? <div style={{ fontSize: 12, color: 'var(--fg-muted)', marginTop: 4 }}>{subtitle}</div> : null}
      </div>
      {link ? (
...
function ResultRow({ game, teamName }) {
  const we = game.us;
  const them = game.them;
  const win = we > them;
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 12px', background: 'var(--bone)', border: '1px solid var(--border)', borderRadius: 8 }}>
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 26, lineHeight: 1, fontVariantNumeric: 'tabular-nums', color: win ? 'var(--court-navy)' : 'var(--fg-muted)' }}>
        {we}<span style={{ color: 'var(--fg-muted)', margin: '0 6px' }}>–</span>{them}
      </div>
    </div>
  );
}
...
function Announce({ from, time, title, body, warn }) {
  return (
    <div style={{ padding: '14px 0', borderBottom: '1px solid var(--border)' }}>
      ...
      <div style={{ fontSize: 13, color: 'var(--fg-soft)', lineHeight: 1.5, textWrap: 'pretty' }}>{body}</div>
    </div>
  );
}
