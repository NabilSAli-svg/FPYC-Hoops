...
import { useBrackets, INITIAL_BRACKETS } from '../shared/store.js';
    ...
    <section style={{ background: '#fff', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '64px 24px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <SectionHead eyebrow={`${bracket.season} Playoffs`} title="Playoff Bracket" />
          {champ && (
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 28 }}>
          {DIV_NAMES.map(d => {
            ...
            const dotColor = b.status === 'complete' ? 'var(--status-win)' : b.status === 'setup' ? 'var(--border)' : 'var(--basketball-orange)';
            return (
              <button key={d} onClick={() => setActiveDiv(d)} style={{
                padding: '7px 16px', borderRadius: 999, border: `1.5px solid ${active ? 'var(--court-navy)' : 'var(--border)'}`,
                ...
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: active ? 'var(--varsity-gold)' : dotColor, flexShrink: 0 }} />
                {d}
              </button>
            );
          })}
        ...
        {status === 'setup' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 8 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--fg)' }}>{bracket.division} · Playoff Seeds</div>
        ...
        {/* Live / complete bracket */}
        {status !== 'setup' && (
          !isMobile ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px 1fr 60px 1fr', gap: 0, alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                {semis.map((s, i) => (
                  <MatchupCard key={s.id} title={`Semifinal ${i + 1}`} subtitle={s.date} topIdx={s.top} bottomIdx={s.bottom} scoreTop={s.scoreTop} scoreBottom={s.scoreBottom} winner={s.winner} seeds={seeds} />
                ))}
              </div>
              ...
              <MatchupCard title="Championship" subtitle={final.date} topIdx={final.top} bottomIdx={final.bottom} scoreTop={final.scoreTop} scoreBottom={final.scoreBottom} winner={final.winner} seeds={seeds} label1="Winner SF1" label2="Winner SF2" />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {semis.map((s, i) => <MatchupCard key={s.id} title={`Semifinal ${i + 1}`} subtitle={s.date} topIdx={s.top} bottomIdx={s.bottom} scoreTop={s.scoreTop} scoreBottom={s.scoreBottom} winner={s.winner} seeds={seeds} />)}
                </div>
              </div>
              <div>
...
function TeamRow({ idx, score, isWinner, isLoser, label, seeds }) {
  ...
  return (
    <div style={{
      ...
      opacity: isLoser ? 0.45 : 1, transition: 'all 200ms',
    }}>
      {team ? (
        <>
          ...
          {score != null && <span style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 700, color: isWinner ? 'var(--court-navy)' : 'var(--fg-muted)', flexShrink: 0 }}>{score}</span>}
          {isWinner && score != null && <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 6px', borderRadius: 999, background: 'var(--varsity-gold)', color: 'var(--court-navy)', flexShrink: 0 }}>ADV</span>}
        </>
      ) : (
...
function MatchupCard({ title, subtitle, topIdx, bottomIdx, scoreTop, scoreBottom, winner, seeds, label1, label2 }) {
  return (
    <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <TeamRow idx={topIdx}    score={scoreTop}    isWinner={winner === topIdx    && winner != null} isLoser={winner != null && winner !== topIdx}    label={label1} seeds={seeds} />
        <div style={{ textAlign: 'center', fontSize: 10, fontWeight: 700, color: 'var(--fg-muted)', letterSpacing: '0.06em' }}>VS</div>
        <TeamRow idx={bottomIdx} score={scoreBottom} isWinner={winner === bottomIdx && winner != null} isLoser={winner != null && winner !== bottomIdx} label={label2} seeds={seeds} />
      </div>
    </div>
  );
}
