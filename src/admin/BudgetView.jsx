...
function EditCell({ value, onSave, locked, directorStyle, treasurerStyle, priorStyle }) {
  ...
  return (
    <span
      onClick={() => { setEditing(true); setDraft(String(value || 0)); }}
      title="Click to edit"
...
export default function BudgetView() {
  ...
  function updateFeeType(id, field, val) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--fg-muted)', marginTop: 2 }}>
            {budget.projectedPlayers} projected players
          </div>
        </div>
        {saved && (
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
        ...
        <SummaryCard
          ...
          value={(netBudget >= 0 ? '+' : '') + fmt(netBudget)}
          accent={netBudget >= 0 ? '#1F8A5B' : '#EF4444'}
          sub={`Actual net: ${(netActual >= 0 ? '+' : '') + fmt(netActual)}`}
        />
        <SummaryCard
          label="Spent to Date"
          value={fmt(calc.totalExpActual)}
          accent="var(--basketball-orange)"
          sub={`${fmt(calc.totalExpBudget - calc.totalExpActual)} remaining`}
        />
      </div>
          ...
          <Card padding={0}>
            <div style={{ padding: '14px 18px 10px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--fg-muted)', marginTop: 2 }}>
                Projected players:&nbsp;
                <EditCell value={budget.projectedPlayers} onSave={updateProjectedPlayers} treasurerStyle />
              </div>
            </div>
              ...
              <thead>
                <tr>
                  ...
                  <th style={{ ...th, background: '#EFF6FF' }}>
                    <span title="Director editable">🔵 # Players</span>
                  </th>
                  <th style={{ ...th, background: '#EEF2FF' }}>
                    <span title="Treasurer editable">💜 Fee</span>
                  </th>
                ...
                {budget.feeTypes.map((ft, i) => (
                  <tr key={ft.id} style={{ background: i % 2 === 0 ? '#fff' : 'var(--bone)' }}>
                    ...
                    <td style={{ ...td, background: '#EFF6FF' }}>
                      <EditCell value={ft.players} onSave={v => updateFeeType(ft.id, 'players', v)} directorStyle />
                    </td>
                    <td style={{ ...td, background: '#EEF2FF' }}>
                      <EditCell value={ft.fee} onSave={v => updateFeeType(ft.id, 'fee', v)} treasurerStyle />
                    </td>
                    <td style={{ ...td, background: '#F3F4F6' }}>
                      <EditCell value={ft.players * ft.fee} onSave={() => {}} locked />
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              {calc.expenses.map((e, i) => {
                ...
                return (
                  <tr key={e.id} style={{ background: i % 2 === 0 ? '#fff' : 'var(--bone)' }}>
                    ...
                    <td style={td}>
                      {isAuto
                        ? <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 999, background: '#F3F4F6', color: 'var(--fg-muted)', border: '1px solid var(--border)' }}>AUTO</span>
                    ...
                    <td style={td}>
                      <EditCell value={e.actual || 0} onSave={v => updateExpense(e.id, 'actual', v)} />
                    </td>
                    <td style={{ ...td, fontWeight: 600, color: remaining < 0 ? '#EF4444' : remaining < e.budgetCalc * 0.1 ? '#F59E0B' : 'var(--fg)' }}>
                      {remaining < 0 ? '-' : ''}{fmt(Math.abs(remaining))}
                    </td>
                    <td style={td}>
                      <ProgressBar value={e.actual || 0} max={e.budgetCalc} danger={remaining < 0} />
                    </td>
                  </tr>
                );
              })}
            ...
            <tfoot>
              <tr style={{ background: 'var(--court-navy)' }}>
                ...
                <td style={{ ...td, border: 'none', fontWeight: 700, fontSize: 14, color: calc.totalExpBudget - calc.totalExpActual < 0 ? '#FCA5A5' : '#86EFAC' }}>
                  {fmt(calc.totalExpBudget - calc.totalExpActual)}
                </td>
      ...
      <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', fontSize: 12, color: 'var(--fg-muted)', padding: '4px 0' }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 14, height: 14, background: '#EEF2FF', border: '1px solid #A5B4FC', borderRadius: 3, display: 'inline-block' }} />
          Treasurer-editable (fees & budgets)
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 14, height: 14, background: '#fff', border: '1px solid var(--border)', borderRadius: 3, display: 'inline-block' }} />
          Editable (actual spend)
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 14, height: 14, background: '#F3F4F6', border: '1px solid var(--border)', borderRadius: 3, display: 'inline-block' }} />
